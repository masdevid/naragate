...
- **No hardcoded credentials or budgets**: the harness injects the Sectors API key and credit budget at runtime. On an MCP harness, `NARAGATE_TOKEN` (minted in the web UI) identifies the account so the run uses that user's own key and credits.
